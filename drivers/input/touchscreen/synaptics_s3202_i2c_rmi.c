...
//When the tp reported some random points, then reset it. add by yubin
#define TP_RANDOM_RUNAWAY
#ifdef TP_RANDOM_RUNAWAY
#include <linux/pcb_version.h>
static int input_no_data = 0;
#define TP_CLIP_REG_OFFSET   35
#define TP_CLIP_REG_VALUE    0x1c
#endif
...
static int synaptics_init_panel(struct synaptics_ts_data *ts)
{
	...
	ret = synaptics_i2c_byte_write(ts, F01_CTRL_DEVICE_CONTROL, 0x80);  //ranfei modify sleep mode
	...
	if(get_pcb_version() < PCB_VERSION_EVT_N1) {
		synaptics_i2c_byte_write(ts, ts->fn11_desc.ctrl_base_addr+TP_CLIP_REG_OFFSET, TP_CLIP_REG_VALUE);
		synaptics_i2c_byte_write(ts, ts->fn11_desc.ctrl_base_addr+TP_CLIP_REG_OFFSET+1, TP_CLIP_REG_VALUE);
		synaptics_i2c_byte_write(ts, ts->fn11_desc.ctrl_base_addr+TP_CLIP_REG_OFFSET+2, TP_CLIP_REG_VALUE);
		synaptics_i2c_byte_write(ts, ts->fn11_desc.ctrl_base_addr+TP_CLIP_REG_OFFSET+3, TP_CLIP_REG_VALUE);
	}
#endif
...
firmware_update:
	...
	else //if (ts->vendor_id == TP_VENDOR_TPK)
	{
		...
		ts->snap_right = max_x * 0/6292;  //104
	}
